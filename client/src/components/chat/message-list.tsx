...
export default function MessageList() {
  const { messages, setMessages, loader, selectedConversationId, setSelectedConversationId } = useChatStore();
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${url}/api/conversations/${selectedConversationId}`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <div className="relative">
      <ScrollArea
        ref={containerRef}
        className="min-h-[200px] pr-4 overflow-y-auto max-h-[60vh]"
      >
