...
const TraibleLoader: React.FC<TraibleLoaderProps> = ({
  ...
  useEffect(() => {
    ...
    const progressInterval = setInterval(
      ...
      duration > 0 ? 100 : 30
    ); // Faster updates for timed version
  ...
  return (
    <div className={containerClasses}>
      ...
      {showLogo && (
        <div className="relative w-32 h-32 mb-6">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div
              className={`${sizes.loader} rounded-full border-4 border-black bg-white flex items-center justify-center`}
              style={{ boxShadow: '0 0 20px rgba(136, 160, 194, 0.5)' }}
            >
              <div
                className={`${sizes.loaderInner} rounded-full bg-white border-2 flex items-center justify-center`}
                style={{ borderColor: 'rgba(136, 160, 194, 0.7)' }}
              >
                {/* Traible logo/icon - using document icon as placeholder */}
                <svg
                  className={`${sizes.icon} text-black`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
            ...
            <>
              <div className="absolute inset-0" style={getOrbitStyle(8)}>
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-4">
                  <div
                    className={`${sizes.fileIcon} rounded-full flex items-center justify-center bg-white text-black border-2`}
                    style={{ borderColor: 'rgba(136, 160, 194, 0.7)' }}
                  >
                    <svg
                      className="w-4 h-4"
      ...
      {showProgress && (
        <div
          className={`${sizes.progressBar} bg-gray-200 border-2 border-white rounded-full overflow-hidden mb-3`}
        >
          <div
