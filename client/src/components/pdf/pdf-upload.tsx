...
export default function PdfUpload() {
  ...
  const { setDocumentId, conversationId, setConversationId, loader, setLoader } = useChatStore();
  ...
  const handleUpload = async () => {
    ...
    setLoader({ processing: true, progress: 0 }); // Start UI loader
    ...
    try {
      ...
      setLoader({ processing: false }); // Stop UI loader
      toast({
        variant: 'destructive',
        title: 'Upload Failed',
  ...
  const pollDocumentStatus = async (documentId: string, token: string) => {
    setLoader({ processing: true, progress: 0 }); // Start loader
